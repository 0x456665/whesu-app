...
impl ChildFormMode {
    fn title(self) -> &'static str {
        match self {
            Self::Add => "Add New Child",
            Self::Edit(_) => "Edit Child Record",
        }
    }
...
impl ChildForm {
    fn set(&mut self, field: ChildFormField, value: String) {
        match field {
            ...
            ChildFormField::PhoneNumber => {
                self.phone_error = validate_phone_field(&value);
            ...
            ChildFormField::EmergencyContactPhone => {
                self.ec_phone_error = validate_phone_field(&value);
    ...
    fn build_child(&self, next_id: u32) -> Result<ChildRecord, String> {
        ...
        let parent_first_name =
            required_value(&self.parent_first_name, "Enter the parent's first name.")?;
        let parent_last_name =
            required_value(&self.parent_last_name, "Enter the parent's last name.")?;
        ...
        if !self.emergency_contact_phone.is_empty()
            && validate_phone_field(&self.emergency_contact_phone).is_some()
        {
...
fn validate_phone_field(phone: &str) -> Option<String> {
    ...
    if is_valid_phone(phone) {
        ...
        Some("Format must be +1(NNN)-NNN-NNNN".to_string())
    }
}
...
impl Default for DaycareApp {
    fn default() -> Self {
        ...
        Self {
            ...
            report_daily_input: sample_data::default_daily_report_date()
                .format("%Y-%m-%d")
                .to_string(),
            report_weekly_input: sample_data::default_week_start()
                .format("%Y-%m-%d")
                .to_string(),
            ...
            settings_current_pw: String::new(),
...
enum Message {
    ...
    ReportDailyInputChanged(String),
    ReportWeeklyInputChanged(String),
    ReportMonthSelected(MonthOption),
    ReportYearChanged(String),
    ReportChildFilterSelected(ChildOption),
    ReportChildFilterCleared,
    GenerateReport,
    ReportExportPathChanged(String),
    ExportPickFolder,
    ExportFolderPicked(String),
    ExportCsv,
    EcSearchChanged(String),
    SettingsCurrentPwChanged(String),
    SettingsNewPwChanged(String),
    SettingsConfirmPwChanged(String),
    ChangePassword,
    Noop,
}
...
fn update(state: &mut DaycareApp, message: Message) -> Task<Message> {
    match message {
        ...
        Message::ReportChildFilterSelected(opt) => {
            state.report_child_filter = Some(opt.id);
            state.report_generated = false;
        }
        Message::ReportChildFilterCleared => {
            state.report_child_filter = None;
            state.report_generated = false;
        }
...
fn view_login(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let logo = column![
        ...
        text("Childcare Management System")
            .size(16)
            .style(|_: &Theme| text::Style {
    ...
    let mut form = column![
        text("Sign In").size(22).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
    ...
    if let Some(err) = &state.login_error {
        form = form.push(
            container(text(err.as_str()).size(14).style(|_: &Theme| text::Style {
    ...
    form = form.push(
        button(text("Sign In").size(16).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }))
        .padding([14, 0])
...
fn view_sidebar(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let title = column![
        ...
        text("Daycare").size(13).style(|_: &Theme| text::Style {
            color: Color::from_rgba(1.0, 1.0, 1.0, 0.7).into()
        }),
    ]
    .spacing(6)
    .align_x(Alignment::Center)
        ...
        nav = nav.push(
            button(text(tab.label()).size(15).style(move |_: &Theme| {
                if is_active {
                    ...
                    text::Style {
                        color: Color::from_rgba(1.0, 1.0, 1.0, 0.75).into(),
                    }
                }
            }))
            .padding([13, 20])
    ...
    nav = nav.push(
        container(
            button(text("Logout").size(14).style(|_: &Theme| text::Style {
                color: Color::from_rgba(1.0, 0.6, 0.6, 1.0).into(),
            }))
            .padding([10, 20])
            .width(Fill)
...
fn view_topbar(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let bar = row![
        text(page_title).size(20).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        container(row![]).width(Fill),
        text(date_str).size(13).style(|_: &Theme| text::Style {
            color: Some(TEXT_MUTED)
        }),
    ]
...
fn view_dashboard(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let quick_actions = card(
        ...
        column![
            ...
            row![
                ...
                action_button(
                    "Emergency Contacts",
                    Message::TabSelected(MainTab::EmergencyContacts)
                ),
            ]
            .spacing(10),
        ]
    ...
    let checked_in_children: Vec<_> = state
        .children
        .iter()
        .filter(|c| attendance::is_checked_in(&state.attendance_records, c.id))
        .collect();
    ...
    if checked_in_children.is_empty() {
        ...
        list_col = list_col.push(table_header_row(&[
            ("Child", 3),
            ("Parent", 3),
            ("Checked In At", 3),
        ]));
    ...
    column![stat_row, quick_actions, card_element(list_col.into())]
        .spacing(20)
        .into()
}
...
fn view_children_roster(state: &DaycareApp) -> Element<'_, Message> {
    ...
    if let Some(child_id) = state.pending_delete_id {
        ...
        layout = layout.push(
            container(
                column![
                    text("Confirm Deletion").size(18).font(Font {
                        weight: iced::font::Weight::Bold,
                        ..Default::default()
                    }),
                    text(format!(
                        "Delete {}? All attendance records will also be removed.",
                        cname
                    ))
                    .size(14),
                    row![
                        button(text("Delete Permanently").size(14))
                            .padding([10, 18])
    ...
    let header = row![
        text("Children Roster").size(20).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        container(row![]).width(Fill),
        button(text("+ Add New Child").size(14).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }))
        .padding([10, 20])
        .style(primary_button_style)
        .on_press(Message::ShowAddChildForm),
    ]
    .align_y(Alignment::Center)
    .spacing(12);
    ...
    if state.children.is_empty() {
        ...
        for (i, child) in state.children.iter().enumerate() {
            ...
            let cid = child.id;
            let actions_row = row![
                button(text("Attend").size(13))
                    .padding([6, 10])
                    .style(secondary_button_style)
                    .on_press(Message::OpenAttendanceForChild(cid)),
                button(text("Edit").size(13))
                    .padding([6, 10])
                    .style(secondary_button_style)
                    .on_press(Message::EditChild(cid)),
                button(text("Delete").size(13))
                    .padding([6, 10])
                    .style(danger_button_style)
                    .on_press(Message::RequestDeleteChild(cid)),
            ]
            .spacing(6);
            let row_elem = row![
...
fn view_child_form_page(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let heading = row![back_btn,].align_y(Alignment::Center).spacing(0);
...
fn view_child_form(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let phone_col: Element<'_, Message> = {
        ...
        if let Some(e) = &state.child_form.phone_error {
            c = c.push(text(e.as_str()).size(12).style(|_: &Theme| text::Style {
                color: Some(DANGER),
            }));
        }
        c.into()
    ...
    let ec_phone_col: Element<'_, Message> = {
        let mut c = column![
            text_input(
                "+1(NNN)-NNN-NNNN",
                &state.child_form.emergency_contact_phone
            )
    ...
    column![
        ...
        row![
            form_field(
                "First Name",
                text_input("First name", &state.child_form.parent_first_name)
            ...
            form_field(
                "Last Name",
                text_input("Last name", &state.child_form.parent_last_name)
        ...
        row![
            ...
            form_field(
                "Email",
                text_input("Email address", &state.child_form.email)
        ...
        row![
            form_field(
                "Contact Name",
                text_input("Full name", &state.child_form.emergency_contact_name)
                    .on_input(|v| Message::ChildFormChanged(
                        ChildFormField::EmergencyContactName,
                        v
                    ))
                    .padding(10)
                    .width(Fill)
        ...
        form_field(
            ...
            .on_input(|v| Message::ChildFormChanged(ChildFormField::Allergies, v))
            .padding(10)
            .width(Fill)
        ),
        actions,
...
fn view_attendance_tab(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let picker = pick_list(
        ...
        |opt| Message::ChildSelected(opt.id),
    )
    ...
    let action_btn = match state.attendance_tab {
        AttendanceTab::CheckIn => {
            ...
            button(text("Check In").size(15).font(Font {
                weight: iced::font::Weight::Bold,
                ..Default::default()
            }))
            .padding([12, 24])
        ...
        AttendanceTab::CheckOut => {
            let enabled = state
                .selected_child_id
                .is_some_and(|id| attendance::is_checked_in(&state.attendance_records, id));
            button(text("Check Out").size(15).font(Font {
                weight: iced::font::Weight::Bold,
                ..Default::default()
            }))
            .padding([12, 24])
    ...
    let actions_card = card(
        state.attendance_tab.label(),
        column![
            subtabs,
            row![
                form_field("Selected Child", picker),
                container(action_btn).padding(iced::Padding {
                    top: 20.0,
                    right: 0.0,
                    bottom: 0.0,
                    left: 0.0
    ...
    column![actions_card, summary, table_card]
        .spacing(20)
        .into()
}
...
fn view_attendance_child_summary(state: &DaycareApp) -> Element<'_, Message> {
    ...
    if let Some(child) = state.selected_child() {
        let sl = state.status_label(child.id);
        let accent = if attendance::is_checked_in(&state.attendance_records, child.id) {
            SUCCESS
        } else {
            TEXT_MUTED
        };
        ...
        if recent.is_empty() {
            ...
            content = content.push(table_header_row(&[
                ("Date", 2),
                ("Check In", 2),
                ("Check Out", 2),
                ("Duration", 3),
...
fn view_check_in_table(state: &DaycareApp) -> Element<'_, Message> {
    ...
    if available.is_empty() {
        return card_element(
            content
                .push(empty_state("All children are currently checked in."))
                .into(),
        );
    }
    content = content.push(table_header_row(&[
    ...
    for (i, child) in available.iter().enumerate() {
        ...
        let row_elem = row![
            ...
            container(
                button(text("Check In").size(13))
                    .padding([6, 14])
                    .style(success_button_style)
                    .on_press(Message::CheckInChild(cid))
            )
            .width(Length::FillPortion(2)),
        ]
        .spacing(12)
        .align_y(Alignment::Center);
        content = content.push(container(row_elem).padding([10, 14]).width(Fill).style(
            move |_: &Theme| table_row_bg(i % 2 == 0, state.selected_child_id == Some(cid)),
        ));
    }
    card_element(content.into())
}
...
fn view_check_out_table(state: &DaycareApp) -> Element<'_, Message> {
    ...
    if checked_in.is_empty() {
        return card_element(
            content
                .push(empty_state("No children are currently checked in."))
                .into(),
        );
    }
    content = content.push(table_header_row(&[
    ...
    for (i, child) in checked_in.iter().enumerate() {
        ...
        let row_elem = row![
            ...
            container(
                button(text("Check Out").size(13))
                    .padding([6, 14])
                    .style(danger_button_style)
                    .on_press(Message::CheckOutChild(cid))
            )
            .width(Length::FillPortion(2)),
        ]
        .spacing(12)
...
fn view_reports_tab(state: &DaycareApp) -> Element<'_, Message> {
    let type_section = column![
        text("1. Report Type").size(16).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        row![
    ...
    let period_section: Element<'_, Message> = match state.report_type {
        ReportType::Daily => column![
            text("2. Date").size(16).font(Font {
                weight: iced::font::Weight::Bold,
                ..Default::default()
            }),
            form_field(
                "Date (YYYY-MM-DD)",
                text_input("YYYY-MM-DD", &state.report_daily_input)
                    .on_input(Message::ReportDailyInputChanged)
                    .padding(10)
                    .width(Fill)
            ),
        ]
        ...
        ReportType::Weekly => column![
            text("2. Week Start").size(16).font(Font {
                weight: iced::font::Weight::Bold,
                ..Default::default()
            }),
            form_field(
                "Week starting (YYYY-MM-DD)",
                text_input("YYYY-MM-DD", &state.report_weekly_input)
                    .on_input(Message::ReportWeeklyInputChanged)
                    .padding(10)
                    .width(Fill)
            ),
        ]
        ...
        ReportType::Monthly => column![
            text("2. Month & Year").size(16).font(Font {
                weight: iced::font::Weight::Bold,
                ..Default::default()
            }),
            row![
                form_field(
                    ...
                    pick_list(
                        MONTHS,
                        Some(state.report_month.clone()),
                        Message::ReportMonthSelected
                    )
                    .width(Fill)
                ),
                form_field(
                    "Year",
                    text_input("YYYY", &state.report_year)
                        .on_input(Message::ReportYearChanged)
                        .padding(10)
                        .width(Fill)
                ),
            ]
    ...
    let mut all_opts = vec![ChildOption {
        id: 0,
        label: "All Children".to_string(),
    }];
    ...
    let sel_opt = state
        ...
        .map(|id| ChildOption {
            id,
            label: state.child_label(id),
        })
        .unwrap_or(ChildOption {
            id: 0,
            label: "All Children".to_string(),
        });

    let filter_section = column![
        text("3. Child Filter (optional)").size(16).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        form_field(
            "Child",
            pick_list(all_opts, Some(sel_opt), |opt| {
                if opt.id == 0 {
                    Message::ReportChildFilterCleared
                } else {
                    Message::ReportChildFilterSelected(opt)
    ...
    let gen_section = column![
        text("4. Generate").size(16).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        button(text("Generate Report").size(15).font(Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }))
        .padding([12, 28])
    ...
    let controls = card(
        "Report Setup",
        column![type_section, period_section, filter_section, gen_section].spacing(20),
    );
    let mut layout = column![controls].spacing(20);
    if state.report_generated {
        layout = layout.push(view_report_results(state));
    }
    layout.into()
}
...
fn view_report_results(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let period_label = match state.report_type {
        ...
        ReportType::Monthly => format!(
            "Monthly Report -- {} {}",
            state.report_month.label, state.report_year
        ),
    };
    ...
    content = content.push(text("Export to CSV").size(16).font(Font {
        weight: iced::font::Weight::Bold,
        ..Default::default()
    }));
    content = content.push(
    ...
    content = content.push(
        row![
            form_field(
                ...
                .on_input(Message::ReportExportPathChanged)
                .padding(10)
                .width(Fill)
            ),
            container(
                row![
                    button(text("Choose Folder...").size(14))
                        .padding([10, 16])
                        .style(secondary_button_style)
                        .on_press(Message::ExportPickFolder),
...
fn rtype_btn(
    ...
    button(text(label).size(14))
        ...
        .on_press(Message::ReportTypeSelected(rt))
        .into()
}
...
fn view_emergency_contacts_tab(state: &DaycareApp) -> Element<'_, Message> {
    ...
    let filtered: Vec<_> = state
        .children
        .iter()
        .filter(|c| q.is_empty() || c.full_name().to_lowercase().contains(&q))
        .collect();

    let mut results = column![table_header_row(&[
        ("Child", 3),
        ("Emergency Contact", 3),
        ("EC Phone", 3),
        ("Parent", 3),
    ...
    if filtered.is_empty() {
        ...
        for (i, c) in filtered.iter().enumerate() {
            let ec_name = if c.emergency_contact_name.is_empty() {
                "--".to_string()
            } else {
                c.emergency_contact_name.clone()
            };
            let ec_phone = if c.emergency_contact_phone.is_empty() {
                "--".to_string()
            } else {
                c.emergency_contact_phone.clone()
            };
            results = results.push(table_row(
                &[
                    ...
                    (ec_name, 3),
                    (ec_phone, 3),
                    (c.parent.full_name(), 3),
                    (c.parent.phone_number.clone(), 3),
                ],
                i % 2 == 0,
                false,
            ));
    ...
    card(
        ...
        column![
            form_field(
                ...
                text_input("Type a name...", &state.ec_search)
                    .on_input(Message::EcSearchChanged)
                    .padding(10)
                    .width(Fill)
            ),
            results,
...
fn view_settings_tab(state: &DaycareApp) -> Element<'_, Message> {
    let mut pw_form = column![
        ...
        form_field(
            "Current Password",
            text_input("Current password", &state.settings_current_pw)
                .secure(true)
                .on_input(Message::SettingsCurrentPwChanged)
                .padding(10)
                .width(Fill)
        ),
        form_field(
            "New Password",
            text_input("New password", &state.settings_new_pw)
                .secure(true)
                .on_input(Message::SettingsNewPwChanged)
                .padding(10)
                .width(Fill)
        ),
        form_field(
            "Confirm New Password",
            text_input("Confirm", &state.settings_confirm_pw)
                .secure(true)
                .on_input(Message::SettingsConfirmPwChanged)
                .padding(10)
                .width(Fill)
        ),
        button(text("Change Password").size(15))
            .padding([11, 22])
            .style(primary_button_style)
    ...
    column![card("Account", pw_form), card("Session", logout_section)]
        .spacing(20)
        .into()
}
...
impl DaycareApp {
    ...
    fn reload_children(&mut self) {
        ...
        if self
            .selected_child_id
            .is_some_and(|id| !self.children.iter().any(|c| c.id == id))
        {
    ...
    fn reload_attendance(&mut self) {
        if let Ok(r) = self.store.load_attendance() {
            self.attendance_records = r;
        }
    }
    ...
    fn selected_child_option(&self) -> Option<ChildOption> {
        self.selected_child_id.map(|id| ChildOption {
            id,
            label: self.child_label(id),
        })
    }
    ...
    fn currently_checked_in_count(&self) -> usize {
        self.children
            .iter()
            .filter(|c| attendance::is_checked_in(&self.attendance_records, c.id))
            .count()
    }
    ...
    fn check_out_child(&mut self, child_id: u32) {
        ...
        match self.store.check_out(child_id, ts) {
            ...
            Err(_) => self.set_status(StatusMessage::error(
                "This child is not currently checked in.",
            )),
        }
    }
        ...
        match self.child_form.build_child(child_id) {
            Ok(child) => match self.child_form_mode {
                ChildFormMode::Add => match self.store.add_child(&child) {
                    ...
                    Err(err) => {
                        self.set_status(StatusMessage::error(format!("Could not add child: {err}")))
                    }
                },
                ChildFormMode::Edit(_) => match self.store.update_child(&child) {
                    ...
                    Err(err) => {
                        self.set_status(StatusMessage::error(format!("Could not update: {err}")))
                    }
                },
            },
    ...
    fn generate_report(&mut self) {
        ...
        match self.report_type {
            ReportType::Daily => {
                match parse_date_value(&self.report_daily_input, "Enter a valid date (YYYY-MM-DD).")
                {
                    Ok(date) => {
                        self.report_rows = reports::daily_report(
                            &self.children,
                            &self.attendance_records,
                            date,
                            filter,
            ...
            ReportType::Weekly => match parse_date_value(
                &self.report_weekly_input,
                "Enter a valid week start date (YYYY-MM-DD).",
            ) {
                Ok(ws) => {
                    self.report_rows = reports::weekly_report(
                        &self.children,
                        &self.attendance_records,
                        ws,
                        filter,
    ...
    fn export_csv(&mut self) {
        ...
        if path.is_empty() {
            self.report_export_status =
                Some("Click 'Choose Folder...' to set an export location first.".to_string());
            return;
        }
    ...
    fn change_password(&mut self) {
        ...
        if new_pw.len() < 6 {
            self.settings_status = Some(StatusMessage::error(
        ...
        match self.store.update_password(&new_pw) {
            Ok(()) => {
                ...
                self.settings_status =
                    Some(StatusMessage::success("Password changed successfully."));
            }
...
fn empty_state(msg: &str) -> Element<'_, Message> {
    container(text(msg).size(15).style(|_: &Theme| text::Style {
        color: Some(TEXT_MUTED),
    }))
    .padding([24, 0])
    .width(Fill)
...
fn table_row_bg(is_even: bool, is_selected: bool) -> container::Style {
    if is_selected {
        container::Style {
            background: Some(iced::Background::Color(SELECTED_ROW)),
            border: iced::border::rounded(8.0),
            ..Default::default()
        }
    } else if is_even {
...
fn danger_button_style(_: &Theme, status: button::Status) -> button::Style {
    let (bg, tc) = match status {
        button::Status::Hovered | button::Status::Pressed => {
            (Color::from_rgb(0.75, 0.15, 0.13), Color::WHITE)
        }
        button::Status::Disabled => (
            Color::from_rgb(0.93, 0.80, 0.80),
            Color::from_rgb(0.72, 0.72, 0.72),
        ),
...
fn success_button_style(_: &Theme, status: button::Status) -> button::Style {
    let (bg, tc) = match status {
        button::Status::Hovered | button::Status::Pressed => {
            (Color::from_rgb(0.2, 0.56, 0.24), Color::WHITE)
        }
        button::Status::Disabled => (
            Color::from_rgb(0.80, 0.93, 0.81),
            Color::from_rgb(0.72, 0.72, 0.72),
        ),
